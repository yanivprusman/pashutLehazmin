...
- "clarify_needed": a short Hebrew question if the item is too ambiguous to shop for. Only set when truly ambiguous (e.g. plain "חלב" with no fat % hint — ask "איזה אחוז שומן?"). The question should NOT list the options — those go in clarify_options. Otherwise omit.
- "clarify_options": array of 2-5 short Hebrew option strings the user can pick from, required whenever clarify_needed is set. Each option should be short enough to fit on a button (1-4 words) and directly usable as a qualifier for the query.

Return ONLY a JSON array, no explanation, no markdown fences. Example:

Input: "2 לחמים, חלב, יוגורט, בננות"
Output: [{"query":"לחם","quantity":2},{"query":"חלב","quantity":1,"clarify_needed":"איזה אחוז שומן?","clarify_options":["1%","3%","נטול לקטוז"]},{"query":"יוגורט","quantity":1,"clarify_needed":"איזה סוג יוגורט?","clarify_options":["טבעי","פירות","יווני"]},{"query":"בננה","quantity":1}]
